...
export class TeditListEditor {
  ...
  createDropdownItem(key, value, index, isColorList) {
    ...
    item.value = value;
    item.innerHTML = key;
    const getOptimalTextColor = (hex) => {
      const luminance = calculateLuminance(hex);
      return luminance > 0.5 ? '#000' : '#FFF';
    };
    if (isColorList) {
      const colorHelper = new Tcolor();
      colorHelper.setHex ? colorHelper.setHex(value) : null;
...
export class TpropEditor extends Twindow {
  constructor(width, height) {
    ...
    cv = document.createElement('div');
    cv.style.cssText = 'height:100%;width:calc(100% - 105px);overflow:hidden;display:inline-block;margin-left:5px;vertical-align:top;';
